		...
		var e = {},
			...
			d = n.WordArray = b.extend({
				...
				concat: function(c) {
					...
					if(f % 4)
						for(var r = 0; r < c; r++)
							a[f + r >>> 2] |= (m[r >>> 2] >>> 24 - 8 * (r % 4) & 255) << 24 - 8 * ((f + r) % 4);
...
(function() {
	...
	s.enc.Base64 = {
		stringify: function(e) {
			...
			e = [];
			for(var d = 0; d < l; d += 3)
				for(var t = (n[d >>> 2] >>> 24 - 8 * (d % 4) & 255) << 16 | (n[d + 1 >>> 2] >>> 24 - 8 * ((d + 1) % 4) & 255) << 8 | n[d + 2 >>> 2] >>> 24 - 8 * ((d + 2) % 4) & 255, q = 0; 4 > q && d + 0.75 * q < l; q++)
...
(function(s) {
	...
	for(var b = CryptoJS, d = b.lib, t = d.WordArray, q = d.Hasher, d = b.algo, a = [], v = 0; 64 > v; v++)
...
CryptoJS.lib.Cipher || function(s) {
		...
		u = (l.pad = {}).Pkcs7 = {
			pad: function(a, f) {
				for(var b = 4 * f, b = b - a.sigBytes % b, c = b << 24 | b << 16 | b << 8 | b, d = [], e = 0; e < b; e += 4)
					d.push(c);
				b = p.create(d, b);
				a.concat(b)
			},
...
(function() {
	...
	var l = CryptoJS,
		...
		p = n.RC4 = e.extend({
			_doReset: function() {
				for(var b = this._key, d = b.words, b = b.sigBytes, e = this._S = [], l = 0; 256 > l; l++)
					e[l] = l;
